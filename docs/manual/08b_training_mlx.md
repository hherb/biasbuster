...
**Recommendations:**
- **64 GB Mac:** Start with `qwen3.5-9b-4bit` for fast iteration. Move to `qwen3.5-9b-8bit` for better quality once workflow is validated.
- **128 GB Mac:** Use `qwen3.5-27b-4bit` for the best quality/speed tradeoff. Use `qwen3.5-27b-8bit` for maximum precision.

> **Note:** OLMo-3.1-32B does not currently have MLX-community pre-quantized weights, so the MLX path focuses on Qwen models.
...
```bash
# Merge (de-quantizes to full precision)
./run_merge_mlx.sh qwen3.5-9b-4bit

# Export to Ollama with GGUF quantization
...
Then continue with [Chapter 9 (Merging, Quantizing & Deploying)](09_merge_and_deploy.md) for GGUF quantization details and Ollama deployment verification, and [Chapter 10 (Evaluating Fine-Tuned Models)](10_evaluation.md) for evaluation.
