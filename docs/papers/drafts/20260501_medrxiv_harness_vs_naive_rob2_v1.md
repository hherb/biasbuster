...
**Decoding regime (locked in pre-reg §5).** All passes use each model's default decoding parameters: Anthropic Sonnet 4.6 at temperature 1.0 / top_p 1.0 (no nucleus filtering); Ollama-served local models (gpt-oss:20b, gemma4, qwen3.6) at temperature 0.8 / top_p 0.9 / top_k 40 (Modelfile defaults). No fixed seed is set. These regimes are genuinely stochastic — at temperature 0 (greedy decoding) run-to-run κ would approach 1.0 vacuously, but at default temperatures the model's signalling-question answers vary stochastically, which is what produces measurable variance for κ to operate on. A planned sensitivity analysis at T={0, 0.3, 0.6, 1.2} on gpt-oss:20b × fulltext (subgroup of n≈10–20 RCTs, three passes each) will quantify the temperature contribution explicitly; results will be reported in supplementary S6. See §5 Limitations for a methodological discussion of the intra-model-vs-inter-rater distinction.
