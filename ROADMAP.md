...
| ✅ Done | κ regeneration + manuscript update | Retro-tag (no-op) → strict+inclusive tables regenerated → both drafts on strict-primary numbers; ensemble now loses for all 4 models (2026-07-17) |
| ✅ Done | Wrong-paper recovery guard | `WRONG_PAPER_RCTS` in `recover_parse_failures.py` + tests; RCT030 rows reverted and documented in `benchmark_rct.notes` (2026-07-17) |
